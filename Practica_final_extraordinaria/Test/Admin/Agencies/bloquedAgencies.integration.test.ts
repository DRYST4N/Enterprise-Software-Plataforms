...
import { BloquedAgencies } from '../../../Backend/src/Admin/Agencies/usecase/bloqued-agencies.usecase.js';
...
class FakeAdminAgenciesRepository implements IAdminAgenciesRepository {
    ...
    async toggleBloqueoAgencia(agenciaId: string, bloqueada: boolean): Promise<any> {
        const agencia = this.agencias.find((a) => a.id === agenciaId);
        if (!agencia) {
            throw new Error('La agencia solicitada no existe.');
        }
        agencia.bloqueada = bloqueada;
        return agencia;
    }
}
    ...
    const dependencies = {
        usecases: {
            AdminAgenciesUseCases: {
                ...
                bloquedAgencies: new BloquedAgencies(repository),
            },
        },
    };
...
describe('PUT /agencies/bloqued-agency/:agenciaId (Admin BloquedAgencies)', () => {
    ...
    it('bloquea una agencia existente', async () => {
        const agencia = seedAgencia(repository);
        const app = buildApp(repository);

        const response = await request(app)
            .put(`/agencies/bloqued-agency/${agencia.id}`)
            .set('x-mock-user', JSON.stringify(adminUser))
            .send({ bloqueada: true });

        expect(response.status).toBe(200);
        expect(response.body.bloqueada).toBe(true);
        expect(repository.agencias[0].bloqueada).toBe(true);
    });

    it('desbloquea una agencia previamente bloqueada', async () => {
        const agencia = seedAgencia(repository);
        agencia.bloqueada = true;
        const app = buildApp(repository);

        const response = await request(app)
            .put(`/agencies/bloqued-agency/${agencia.id}`)
            .set('x-mock-user', JSON.stringify(adminUser))
            .send({ bloqueada: false });

        expect(response.status).toBe(200);
        expect(response.body.bloqueada).toBe(false);
        expect(repository.agencias[0].bloqueada).toBe(false);
    });
    ...
    it('devuelve 400 si la agencia no existe', async () => {
        ...
        const response = await request(app)
            .put('/agencies/bloqued-agency/no-existe')
            .set('x-mock-user', JSON.stringify(adminUser))
            .send({ bloqueada: true });
