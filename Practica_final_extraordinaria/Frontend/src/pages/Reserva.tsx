...
import { paymentAPI } from '../services/api'; // Puerto 5000
...
export default function Reserva() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Ciframos localmente los datos sensibles antes de transmitirlos por HTTP
            ...
            const payload = {
                ...
                checkOut,
                cardHolder,
                cardNumber: tarjetaCifrada,
                expiryDate,                 
                cvv: cvvCifrado             
            };

            // Enviamos la petición a la API de pagos (Puerto 5000)
            await paymentAPI.post('/reservas', payload);
            
            alert('¡Reserva confirmada y pago aprobado con éxito! Redirigiéndote a tus localizadores...');
            
            // 🔥 CORREGIDO: Redirección inmediata al historial para que consulte su QR allí
            navigate('/mis-reservas');

        } catch (err: any) {
            console.error('Error procesando reserva en API de pagos:', err);
            const msg = err.response?.data?.error || err.response?.data?.errors?.[0] || 'Error al procesar el pago.';
            setError(msg);
        } finally {
            setLoading(false);
        }
