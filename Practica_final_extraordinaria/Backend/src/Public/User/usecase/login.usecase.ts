...
import { BadRequestError, ForbiddenError } from "../../../middlewares/Errors/CustomErrors.js";
...
export class LoginUseCase {
    ...
    async execute(input: LoginInput){
        ...
        if(!input.email || !input.passwordPlano){
            throw new BadRequestError('El email y la contraseña son datos obligatorios.');
        }

        const user = await this.authRepository.findByEmail(input.email);
        if(!user) {
            throw new BadRequestError("Las credenciales introducidas no son correctas.");
        }

        const passwordCorrecto = await bcrypt.compare(input.passwordPlano, user.password);
        if(!passwordCorrecto) {
            throw new BadRequestError("Las credenicales introducidas no son correctas.");
        }

        if(user.role === 'AGENCIA' && user.agencia?.bloqueada){
            throw new ForbiddenError("Esa cuenta de agencia se encuentra bloqueada por el Administrador.");
        }
