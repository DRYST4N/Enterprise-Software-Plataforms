import { BadRequestError, NotFoundError, UnauthorizedError } from "../../../middlewares/Errors/CustomErrors.js";
...
export class DeleteApartment {
    ...
    async execute(id: string, agenciaId: string){
        ...
        if(!apto) throw new NotFoundError('Alojamiento no existe.');
        if(apto.agenciaId !== agenciaId) throw new UnauthorizedError('No tienes permisos para eliminar este apartamento.');
        ...
        if(reservasConflictivas.length > 0) {
            throw new BadRequestError('No se puede dar de baja este alojamoento: existen reservas activas o estan programadas para los 3 proximos días.');
        }
